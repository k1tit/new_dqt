...
class FastDataQualityChecker:
    ...
    def _print_table_header(self, table_name, rule_count, row_count):
        ...
        print(f'  Правил: {rule_count:3d} | Строк: {row_count:,} | Начало: {started}')
        print('-' * 100)
    ...
    def _print_table_summary(self, table_name, elapsed_time):
        ...
        print(f'  Успешно:    {success_count:3d} ({success_percent:.1f}%)')
    ...
    def run_quality_checks_fast(self, specific_table: str=None):
        ...
        std_methods = ', '.join(self.FORCE_STANDARD_METHOD[:5])
        print(f'[INFO] Стандартный метод для: {std_methods}...')
        ...
        print(f'\n[DEBUG] ПРОВЕРКА ДОСТУПНЫХ ТАБЛИЦ:')
        for table in ['BUT000', 'KNVP', 'KNB1', 'KNB5', 'DFKKBPTAXNUM']:
            if table in available_tables:
                df = self.memory_manager.get_table(table)
                if df is not None:
                    print(f'{table}: {len(df):,} строк, {len(df.columns)} колонок')
                    col_suffix = '...' if len(df.columns) > 10 else ''
                    print(f'Колонки: {list(df.columns)[:10]}{col_suffix}')
            else:
    ...
    def _process_with_standard_method_forced(self, table_name, df, table_rules, timestamp):
        ...
        for i, rule in enumerate(table_rules, 1):
            ...
            matched_column = self._find_column_with_mapping(table_name, df.columns, column_to_check)
            if not matched_column:
                matched_column = self.column_matcher.find_column_match(df.columns, column_to_check)
            if not matched_column:
                matched_column = self._find_most_similar_column(df.columns, column_to_check)
            sys.stdout.write(f'\r    [{i:3d}/{total_rules:3d}] {rule_code:20} | ')
            if matched_column:
                sys.stdout.write(f'Используем колонку: {matched_column}')
            else:
                sys.stdout.write(f'Колонка не найдена: {column_to_check}')
            sys.stdout.flush()
    ...
    def _execute_real_check(self, rule, table_name, df, matched_column, timestamp):
        ...
        if rule_code == 'RCCONF_15.1' and str(table_name or '').strip().upper() == 'BUT000':
            org3_res = self._find_column_with_mapping(table_name, df.columns, 'NAME_ORG3')
            if org3_res:
    ...
    def _find_column_with_mapping(self, table_name, available_columns, target_column):
        ...
        for col in available_columns:
            col_upper = col.upper()
            target_for_match = target_upper.replace('_', '').replace('-', '')
            col_for_match = col_upper.replace('_', '').replace('-', '')
    ...
    def _extract_second_column_from_description(self, rule_code, rule_description, columns, first_column):
        ...
        if 'name_org1' in desc_lower and 'name_org2' in desc_lower:
            if 'name_org1' in first_column.lower():
                return self._find_column_with_mapping(None, columns, 'NAME_ORG2')
            else:
                return self._find_column_with_mapping(None, columns, 'NAME_ORG1')
        if 'name_org1' in desc_lower and ('name_org3' in desc_lower or 'name_org4' in desc_lower):
            if 'name_org1' in first_column.lower():
                if 'name_org3' in desc_lower:
                    return self._find_column_with_mapping(None, columns, 'NAME_ORG3')
                else:
                    return self._find_column_with_mapping(None, columns, 'NAME_ORG4')
        if 'taxnum' in desc_lower:
            if '1' in first_column.lower():
                return self._find_column_with_mapping(None, columns, 'TAXNUM2')
            elif '2' in first_column.lower():
                return self._find_column_with_mapping(None, columns, 'TAXNUM1')
        return None
    ...
    def _print_final_statistics(self):
        ...
        if suspicious_list:
            ...
            for rule in suspicious_list[:10]:
                ...
                print(f'  • {rc:20} - {tn:15} - {failed:,} ошибок')
        overall_time = time.time() - self.start_time
